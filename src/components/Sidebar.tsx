...
const MenuButton = styled.button<{ isActive?: boolean }>`
  ...
  height: 40px;
  border: none;
  background: ${props => props.isActive ? theme.colors.primary : 'none'};
  color: #ffffff;
  font-size: 16px;
  cursor: pointer;
  margin-bottom: 20px;
  ...
  &:hover {
    background-color: ${props => props.isActive ? theme.colors.primaryHover : '#4a4a4a'};
  }
`;
...
const Sidebar: React.FC<SidebarProps> = ({ viewMode, onViewModeChange }) => {
  return (
    <SidebarContainer>
      ...
      <MenuButton 
        title="조직도" 
        isActive={viewMode === 'organization'}
